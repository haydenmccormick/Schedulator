...
function getConfig(projectRoot, options = {}) {
    ...
    const [packageJson, packageJsonPath] = getPackageJsonAndPath(projectRoot, jsonFileWithNodeModulesPath.expo);
 ...
 * @param projectRoot
 */
function ensureInternals(config, internals) {
    if (!config._internal) {
        config._internal = {};
    }
    config._internal = Object.assign(Object.assign({ isDebug: EXPO_DEBUG }, config._internal), internals);
    return config;
}
...
function ensureConfigHasDefaultValues({ projectRoot, exp, pkg, paths, packageJsonPath, skipSDKVersionRequirement = false, }) {
    ...
    exp = ensureInternals(exp, Object.assign(Object.assign({ projectRoot }, (paths !== null && paths !== void 0 ? paths : {})), { packageJsonPath }));
